...
def masked_mean(values: Tensor, mask: Tensor, axis: int | None = None, denominator: float | None = None) -> Tensor:
...
class MetricsTracker:
    ...
    def add(self, name: str, value: Tensor):
...
def collate_fn(tensors_list: list[Tensor], pad_token_id: int, pin_memory: bool = True) -> Tensor:
...
def to_device_inplace(tensors_list: list[Tensor], device: torch.device):
...
class PolicyTrainerRayProcess(RayProcess):
    def from_pretrained(self, args: Args, model_config: ModelConfig, tokenizer: PreTrainedTokenizer):
        ...
        self.optimizer = torch.optim.AdamW(optim_params, lr=args.learning_rate, fused=True)
...
def run_training(
    ...
    save_final_model(args, policy_group, tokenizer, training_step, tc.chat_template_name)
